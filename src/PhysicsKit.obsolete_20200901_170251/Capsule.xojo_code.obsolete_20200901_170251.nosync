...
Implements PhysicsKit.Convex
	...
	#tag Method, Flags = &h0
		Function Contains(point As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As Boolean
		  ...
		  Var p As Vector2 = Segment.GetPointOnSegmentClosestToPoint(point, _
	...
	#tag Method, Flags = &h0
		Function CreateAABB(transform As PhysicsKit.Transform) As PhysicsKit.AABB
		  ...
		  Var p1 As Vector2 = Self.getFarthestPoint(Vector2.X_AXIS, transform)
		  ...
		  p1 = Self.GetFarthestPoint(Vector2.Y_AXIS, transform)
	...
	#tag Method, Flags = &h0
		Function GetAxes(foci() As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As PhysicsKit.Vector2()
		  ...
		  If foci <> Nil Then
		    ...
		    Var f1 As Vector2 = transform.GetTransformed(Self.Foci(0))
		    Var f2 As Vector2 = transform.GetTransformed(Self.foci(1))
	...
	#tag Method, Flags = &h0
		Function GetFarthestFeature(vector As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As PhysicsKit.Feature
		  ...
		  Var n1 As Vector2 = Self.LocalXAxis.GetLeftHandOrthogonalVector
		  ...
		  If Abs(d1) < d Then
		    ...
		    Var point As Vector2 = Self.GetFarthestPoint(vector, transform)
		    Return New PointFeature(point)
		  Else
		    // Compute the vector to add/sub from the foci.
		    Var v As Vector2 = n1.Multiply(Self.CapRadius)
		    
		    // Compute an expansion amount based on the width of the shape.
		    Var e As Vector2 = Self.LocalXAxis.Product(Self.Length * 0.5 * EDGE_FEATURE_EXPANSION_FACTOR)
		    If d1 > 0 Then
		      Var p1 As Vector2 = Self.Foci(0).Sum(v).Subtract(e)
		      Var p2 As Vector2 = Self.Foci(1).Sum(v).Add(e)
		      
		      // Return the full bottom side.
		      Return Segment.GetFarthestFeature(p1, p2, vector, transform)
		    Else
		      Var p1 As Vector2 = Self.Foci(0).Difference(v).Subtract(e)
		      Var p2 As Vector2 = Self.Foci(1).Difference(v).Add(e)
		      Return Segment.GetFarthestFeature(p1, p2, vector, transform)
		    End If
		  End If
	...
	#tag Method, Flags = &h0
		Function GetFarthestPoint(vector As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As PhysicsKit.Vector2
		  ...
		  // A capsule is just a radially expanded line segment.
		  Var p As Vector2 = Segment.GetFarthestPoint(Self.Foci(0), Self.Foci(1), vector, transform)
