...
Implements PhysicsKit.Convex
	#tag Method, Flags = &h21
		Private Sub Constructor(valid As Boolean, radius As Double, theta As Double, center As PhysicsKit.Vector2)
		  ///
	...
	#tag Method, Flags = &h0
		Function Contains(point As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As Boolean
		  ...
		  Var lp As Vector2 = transform.GetInverseTransformed(point)
		  ...
		  Var v As Vector2 = Self.Vertices(0).Towards(lp)
	...
	#tag Method, Flags = &h0
		Function CreateAABB(transform As PhysicsKit.Transform) As PhysicsKit.AABB
		  ...
		  Var minX As Double = Self.GetFarthestPoint(Vector2.INV_X_AXIS, transform).X
		  Var maxX As Double = Self.GetFarthestPoint(Vector2.X_AXIS, transform).X
		  
		  // Inlined projection of y axis
		  Var minY As Double = Self.GetFarthestPoint(Vector2.INV_Y_AXIS, transform).Y
		  Var maxY As Double = Self.GetFarthestPoint(Vector2.Y_AXIS, transform).Y
	...
	#tag Method, Flags = &h0
		Function GetAxes(foci() As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As PhysicsKit.Vector2()
		  ...
		  Var focus As Vector2 = transform.GetTransformed(Self.Vertices(0))
		  
		  Var iLimit As Integer = fociSize - 1
		  For i As Integer = 0 To iLimit
		    // Get the current focus.
		    Var f As Vector2 = foci(i)
		    
		    // Create a place for the closest point.
		    Var closest As Vector2 = focus
	...
	#tag Method, Flags = &h0, Description = 52657475726E732074686520746970206F6620746865207069652073686170652E2054686973206973207468652063656E747265206F662074686520636972636C652E
		Function GetCircleCenter() As PhysicsKit.Vector2
		  ///
	...
	#tag Method, Flags = &h0
		Function GetFarthestPoint(vector As PhysicsKit.Vector2, transform As PhysicsKit.Transform) As PhysicsKit.Vector2
		  ...
		  If localnRotated.X < Self.CosAlpha Then
		    ...
		    Var point As Vector2 = New Vector2(Self.Vertices(maxIndex))
	...
	#tag Method, Flags = &h0
		Sub Rotate(r As PhysicsKit.Rotation, x As Double, y As Double)
		  ...
		  For Each n As Vector2 In Self.Normals
		    Call n.Rotate(r)
		  Next n
